(function (factory) {
    ...
    function eatQuoted(stream, options) {
        ...
        if (stream.eat(isQuote)) {
            ...
            // If weвЂ™re here then stream wasnвЂ™t properly consumed.
     ...
     * @return Returns `true` if character pair was successfully consumed, itвЂ™s
    ...
    function eatPair(stream, open, close, options) {
        ...
        if (stream.eat(open)) {
            ...
            // If weвЂ™re here then paired character canвЂ™t be consumed
            stream.pos = start;
            if (options.throws) {
    ...
    function attribute(scanner) {
        if (quoted(scanner)) {
            // Consumed quoted value: itвЂ™s a value for default attribute
            return {
                value: slice(scanner)
            };
        }
    ...
    function literal(scanner, allowBrackets) {
        ...
        while (readable(scanner)) {
            ...
            if (brackets.expression) {
                // If weвЂ™re inside expression, we should consume all content in it
    ...
    function field(scanner, ctx) {
        ...
        // If we reached here then thereвЂ™s no valid field here, revert
    ...
    /**
     * Check if given character code is an operator and itвЂ™s allowed in current context
     */
    ...
    /**
     * If given character is a bracket, returns itвЂ™s type
     */
    ...
    /**
     * If given character is an operator, returns itвЂ™s type
     */
    ...
    const tokenVisitor = {
        ...
        Field(token, state) {
            if (token.index != null) {
                // ItвЂ™s a field: by default, return TextMate-compatible field
                return token.name
        ...
        if (node.repeat) {
            ...
            for (let i = 0; i < repeat.count; i++) {
                ...
                if (repeat.implicit && !state.inserted) {
                    // ItвЂ™s an implicit repeater but no repeater placeholders found inside,
                ...
                // We should output at least one repeated item even if itвЂ™s reached
                // repeat limit
    ...
    function convertElement(node, state) {
        ...
        if (!elem.name && !elem.attributes && elem.value && !elem.value.some(isField)) {
            // XXX itвЂ™s unclear that `children` is not bound to `elem`
    ...
    function convertAttribute(node, state) {
        ...
        if (node.value) {
            ...
            if (isQuote$1(tokens[0])) {
                // ItвЂ™s a quoted value: remove quotes from output but mark attribute
                // value as quoted
    ...
    function stringifyValue(tokens, state) {
        ...
        for (let i = 0, token; i < tokens.length; i++) {
            ...
            if (isField(token)) {
                ...
                // own syntax for field or doesnвЂ™t support fields at all so we should
     ...
     * The difference between вЂњshortвЂќ and вЂњfullвЂќ notation is that first one uses
     * alpha characters only and used for extracting keywords from abbreviation,
     * while вЂњfullвЂќ notation also supports numbers and dashes
     */
    ...
    function consumeNumber(stream) {
        ...
        if (stream.eat(46 /* Dot */)) {
            // ItвЂ™s perfectly valid to have numbers like `1.`, which enforces
    ...
    function attributeName(scanner) {
        ...
        // React-style вЂ“ `<div {...props}>`
        // Angular-style вЂ“ `<div [ng-for]>` or `<div *ng-for>`
     ...
     * Unlike regular scanner, fast scanner doesnвЂ™t provide info about attributes to
     * reduce object allocations hence increase performance.
     * If `callback` returns `false`, scanner stops parsing.
     * @param special List of вЂњspecialвЂќ HTML tags which should be ignored. Most likely
     * itвЂ™s a "script" and "style" tags.
     */
    ...
    function match(source, pos, opt) {
        // Since we expect large input document, weвЂ™ll use pooling technique
     ...
     * It doesnвЂ™t provide detailed info about CSS atoms like compound selectors,
    ...
    function scan$1(source, callback) {
        ...
        while (!scanner.eof()) {
            ...
            if ((blockEnd = scanner.eat(125 /* RightCurly */)) || scanner.eat(59 /* Semicolon */)) {
                ...
                // вЂ” property delimiter: `foo: bar`, must be in block context
                // вЂ” variable delimiter: `$foo: bar`, could be anywhere
                // вЂ” pseudo-selector: `a:hover`, could be anywhere (for LESS and SCSS)
                // вЂ” media query expression: `min-width: 100px`, must be inside expression context
                // Since I canвЂ™t easily detect `:` meaning for sure, weвЂ™ll update state
        ...
        if (state.start !== -1) {
            // ThereвЂ™s pending token in state
     ...
     * ItвЂ™s possible that comment may not have closing part
     */
    ...
    function balancedInward$1(source, pos) {
        ...
        /**
         * Pushes given inward range as a first child of current selector only if itвЂ™s
         * not set yet
         */
    ...
    function attributeValueRange(tag, attr, offset = 0) {
        ...
        if (isQuote$2(tag[valueStart])) {
            valueStart++;
        }
        if (isQuote$2(tag[valueEnd - 1]) && valueEnd > valueStart) {
            valueEnd--;
        }
    ...
    var ru = {
        "common": ["РґР°Р»РµРєРѕ-РґР°Р»РµРєРѕ", "Р·Р°", "СЃР»РѕРІРµСЃРЅС‹РјРё", "РіРѕСЂР°РјРё", "РІ СЃС‚СЂР°РЅРµ", "РіР»Р°СЃРЅС‹С…", "Рё СЃРѕРіР»Р°СЃРЅС‹С…", "Р¶РёРІСѓС‚", "СЂС‹Р±РЅС‹Рµ", "С‚РµРєСЃС‚С‹"],
        "words": ["РІРґР°Р»Рё", "РѕС‚ РІСЃРµС…", "РѕРЅРё", "Р±СѓРєРІРµРЅРЅС‹С…", "РґРѕРјР°С…", "РЅР° Р±РµСЂРµРіСѓ", "СЃРµРјР°РЅС‚РёРєР°",
            "Р±РѕР»СЊС€РѕРіРѕ", "СЏР·С‹РєРѕРІРѕРіРѕ", "РѕРєРµР°РЅР°", "РјР°Р»РµРЅСЊРєРёР№", "СЂСѓС‡РµРµРє", "РґР°Р»СЊ",
            "Р¶СѓСЂС‡РёС‚", "РїРѕ РІСЃРµР№", "РѕР±РµСЃРїРµС‡РёРІР°РµС‚", "РµРµ","РІСЃРµРјРё", "РЅРµРѕР±С…РѕРґРёРјС‹РјРё",
            "РїСЂР°РІРёР»Р°РјРё", "СЌС‚Р°", "РїР°СЂР°РґРёРіРјР°С‚РёС‡РµСЃРєР°СЏ", "СЃС‚СЂР°РЅР°", "РєРѕС‚РѕСЂРѕР№", "Р¶Р°СЂРµРЅРЅС‹Рµ",
            "РїСЂРµРґР»РѕР¶РµРЅРёСЏ", "Р·Р°Р»РµС‚Р°СЋС‚", "РїСЂСЏРјРѕ", "СЂРѕС‚", "РґР°Р¶Рµ", "РІСЃРµРјРѕРіСѓС‰Р°СЏ",
            "РїСѓРЅРєС‚СѓР°С†РёСЏ", "РЅРµ", "РёРјРµРµС‚", "РІР»Р°СЃС‚Рё", "РЅР°Рґ", "СЂС‹Р±РЅС‹РјРё", "С‚РµРєСЃС‚Р°РјРё",
            "РІРµРґСѓС‰РёРјРё", "Р±РµР·РѕСЂС„РѕРіСЂР°С„РёС‡РЅС‹Р№", "РѕР±СЂР°Р·", "Р¶РёР·РЅРё", "РѕРґРЅР°Р¶РґС‹", "РѕРґРЅР°",
            "РјР°Р»РµРЅСЊРєР°СЏ", "СЃС‚СЂРѕС‡РєР°","СЂС‹Р±РЅРѕРіРѕ", "С‚РµРєСЃС‚Р°", "РёРјРµРЅРё", "lorem", "ipsum",
            "СЂРµС€РёР»Р°", "РІС‹Р№С‚Рё", "Р±РѕР»СЊС€РѕР№", "РјРёСЂ", "РіСЂР°РјРјР°С‚РёРєРё", "РІРµР»РёРєРёР№", "РѕРєСЃРјРѕРєСЃ",
            "РїСЂРµРґСѓРїСЂРµР¶РґР°Р»", "Рѕ", "Р·Р»С‹С…", "Р·Р°РїСЏС‚С‹С…", "РґРёРєРёС…", "Р·РЅР°РєР°С…", "РІРѕРїСЂРѕСЃР°",
            "РєРѕРІР°СЂРЅС‹С…", "С‚РѕС‡РєР°С…", "Р·Р°РїСЏС‚РѕР№", "РЅРѕ", "С‚РµРєСЃС‚", "РґР°Р»", "СЃР±РёС‚СЊ",
            "СЃРµР±СЏ", "С‚РѕР»РєСѓ", "РѕРЅ", "СЃРѕР±СЂР°Р»", "СЃРµРјСЊ", "СЃРІРѕРёС…", "Р·Р°РіР»Р°РІРЅС‹С…", "Р±СѓРєРІ",
            "РїРѕРґРїРѕСЏСЃР°Р»", "РёРЅРёС†РёР°Р»", "Р·Р°", "РїРѕСЏСЃ", "РїСѓСЃС‚РёР»СЃСЏ", "РґРѕСЂРѕРіСѓ",
            "РІР·РѕР±СЂР°РІС€РёСЃСЊ", "РїРµСЂРІСѓСЋ", "РІРµСЂС€РёРЅСѓ", "РєСѓСЂСЃРёРІРЅС‹С…", "РіРѕСЂ", "Р±СЂРѕСЃРёР»",
            "РїРѕСЃР»РµРґРЅРёР№", "РІР·РіР»СЏРґ", "РЅР°Р·Р°Рґ", "СЃРёР»СѓСЌС‚", "СЃРІРѕРµРіРѕ", "СЂРѕРґРЅРѕРіРѕ", "РіРѕСЂРѕРґР°",
            "Р±СѓРєРІРѕРіСЂР°Рґ", "Р·Р°РіРѕР»РѕРІРѕРє", "РґРµСЂРµРІРЅРё", "Р°Р»С„Р°РІРёС‚", "РїРѕРґР·Р°РіРѕР»РѕРІРѕРє", "СЃРІРѕРµРіРѕ",
            "РїРµСЂРµСѓР»РєР°", "РіСЂСѓСЃС‚РЅС‹Р№", "СЂРµС‚РѕСЂРёС‡РµСЃРєРёР№", "РІРѕРїСЂРѕСЃ", "СЃРєР°С‚РёР»СЃСЏ", "РµРіРѕ",
            "С‰РµРєРµ", "РїСЂРѕРґРѕР»Р¶РёР»", "СЃРІРѕР№", "РїСѓС‚СЊ", "РґРѕСЂРѕРіРµ", "РІСЃС‚СЂРµС‚РёР»", "СЂСѓРєРѕРїРёСЃСЊ",
            "РѕРЅР°", "РїСЂРµРґСѓРїСЂРµРґРёР»Р°",  "РјРѕРµР№", "РІСЃРµ", "РїРµСЂРµРїРёСЃС‹РІР°РµС‚СЃСЏ", "РЅРµСЃРєРѕР»СЊРєРѕ",
            "СЂР°Р·", "РµРґРёРЅСЃС‚РІРµРЅРЅРѕРµ", "С‡С‚Рѕ", "РјРµРЅСЏ", "РѕСЃС‚Р°Р»РѕСЃСЊ", "СЌС‚Рѕ", "РїСЂРёСЃС‚Р°РІРєР°",
            "РІРѕР·РІСЂР°С‰Р°Р№СЃСЏ", "С‚С‹", "Р»СѓС‡С€Рµ", "СЃРІРѕСЋ", "Р±РµР·РѕРїР°СЃРЅСѓСЋ", "СЃС‚СЂР°РЅСѓ", "РїРѕСЃР»СѓС€Р°РІС€РёСЃСЊ",
            "СЂСѓРєРѕРїРёСЃРё", "РЅР°С€", "РїСЂРѕРґРѕР»Р¶РёР»", "СЃРІРѕР№", "РїСѓС‚СЊ", "РІСЃРєРѕСЂРµ", "РµРјСѓ",
            "РїРѕРІСЃС‚СЂРµС‡Р°Р»СЃСЏ", "РєРѕРІР°СЂРЅС‹Р№", "СЃРѕСЃС‚Р°РІРёС‚РµР»СЊ", "СЂРµРєР»Р°РјРЅС‹С…", "С‚РµРєСЃС‚РѕРІ",
            "РЅР°РїРѕРёРІС€РёР№", "СЏР·С‹РєРѕРј", "СЂРµС‡СЊСЋ", "Р·Р°РјР°РЅРёРІС€РёР№", "СЃРІРѕРµ", "Р°РіРµРЅС‚СЃС‚РІРѕ",
            "РєРѕС‚РѕСЂРѕРµ", "РёСЃРїРѕР»СЊР·РѕРІР°Р»Рѕ", "СЃРЅРѕРІР°", "СЃРЅРѕРІР°", "СЃРІРѕРёС…", "РїСЂРѕРµРєС‚Р°С…",
            "РµСЃР»Рё", "РїРµСЂРµРїРёСЃР°Р»Рё", "С‚Рѕ", "Р¶РёРІРµС‚", "С‚Р°Рј", "РґРѕ", "СЃРёС…", "РїРѕСЂ"]
    };

    var sp = {
        "common": ["mujer", "uno", "dolor", "mГЎs", "de", "poder", "mismo", "si"],
        "words": ["ejercicio", "preferencia", "perspicacia", "laboral", "paГ±o",
            "suntuoso", "molde", "namibia", "planeador", "mirar", "demГЎs", "oficinista", "excepciГіn",
            "odio", "consecuencia", "casi", "auto", "chicharra", "velo", "elixir",
            "ataque", "no", "odio", "temporal", "cuГіrum", "dignГ­simo",
            "facilismo", "letra", "nihilista", "expediciГіn", "alma", "alveolar", "aparte",
            "leГіn", "animal", "como", "paria", "belleza", "modo", "natividad",
            "justo", "ataque", "sГ©quito", "pillo", "sed", "ex", "y", "voluminoso",
            "temporalidad", "verdades", "racional", "asunciГіn", "incidente", "marejada",
            "placenta", "amanecer", "fuga", "previsor", "presentaciГіn", "lejos",
            "necesariamente", "sospechoso", "adiposidad", "quindГ­o", "pГіcima",
            "voluble", "dГ©bito", "sintiГі", "accesorio", "falda", "sapiencia",
            "volutas", "queso", "permacultura", "laudo", "soluciones", "entero",
            "pan", "litro", "tonelada", "culpa", "libertario", "mosca", "dictado",
            "reincidente", "nascimiento", "dolor", "escolar", "impedimento", "mГ­nima",
            "mayores", "repugnante", "dulce", "obcecado", "montaГ±a", "enigma",
            "total", "deletГ©reo", "dГ©cima", "cГЎbala", "fotografГ­a", "dolores",
            "molesto", "olvido", "paciencia", "resiliencia", "voluntad", "molestias",
            "magnГ­fico", "distinciГіn", "ovni", "marejada", "cerro", "torre", "y",
            "abogada", "manantial", "corporal", "agua", "crepГєsculo", "ataque", "desierto",
            "laboriosamente", "angustia", "afortunado", "alma", "encefalograma",
            "materialidad", "cosas", "o", "renuncia", "error", "menos", "conejo",
            "abadГ­a", "analfabeto", "remo", "fugacidad", "oficio", "en", "almГЎcigo", "vos", "pan",
            "represiГіn", "nГєmeros", "triste", "refugiado", "trote", "inventor",
            "corchea", "repelente", "magma", "recusado", "patrГіn", "explГ­cito",
            "paloma", "sГ­ndrome", "inmune", "autoinmune", "comodidad",
            "ley", "vietnamita", "demonio", "tasmania", "repeler", "apГ©ndice",
            "arquitecto", "columna", "yugo", "computador", "mula", "a", "propГіsito",
            "fantasГ­a", "alias", "rayo", "tenedor", "deleznable", "ventana", "cara",
            "anemia", "corrupto"]
    };
    ...
    function expandShortNotation(node, ancestors, config) {
        ...
        for (let cl of data.classNames) {
            ...
            if (cl === originalClass) {
                // class name wasnвЂ™t modified: itвЂ™s not a BEM-specific class,
     ...
     * @param items List of `node`вЂ™s siblings
    ...
    /**
     * Outputs given attributeвЂ™s content into output stream
     */
    function pushAttribute(attr, state) {
        ...
        if (attr.name) {
            ...
            if (isBooleanAttribute(attr, config) && !value) {
                // If attribute value is omitted and itвЂ™s a boolean value, check for
                // `compactBoolean` option: if itвЂ™s disabled, set value to attribute name
                // (XML style)
    ...
    function shouldFormat(node, index, items, state) {
        ...
        if (isInline(node, config)) {
            ...
            if (index === 0) {
                // First node in parent: format if itвЂ™s followed by a block-level element
     ...
     * How matching works:
     * вЂ“ first characters of both `str1` and `str2` *must* match
     * вЂ“ `str1` length larger than `str2` length is allowed only when `unmatched` is true
     * вЂ“ ideal match is when `str1` equals to `str2` (score: 1)
     * вЂ“ next best match is `str2` starts with `str1` (score: 1 Г— percent of matched characters)
     * вЂ“ other scores depend on how close characters of `str1` to the beginning of `str2`
    ...
    function scoreMatch(str1, str2, partialMatch = false) {
        ...
        // have higher score.
        // For example, if `str2` is `abcde`, itвЂ™s max score is:
    ...
    function property(node, out, config) {
        ...
        if (node.name) {
            ...
            // ItвЂ™s a regular snippet, output plain tokens without any additional formatting
    ...
    function resolveAsProperty(node, snippet, config) {
        ...
        if (node.value.length) {
            ...
            // We should auto-select inserted value only if thereвЂ™s multiple value
            // choice
    ...
    /**
     * Returns a part of `abbr` that wasnвЂ™t directly matched against `str`.
     * For example, if abbreviation `poas` is matched against `position`,
     * the unmatched part will be `as` since `a` wasnвЂ™t found in string stream
     */
    ...
    /**
     * вЂњPeeksвЂќ character code an current scanner location without advancing it
     */
    ...
    /**
     * Check if given readerвЂ™s current position points at the end of HTML tag
     */
    ...
    function isUnquotedValue(ch) {
        return !isNaN(ch) && ch !== 61 /* Equals */ && !isWhiteSpace$3(ch) && !isQuote$3(ch);
    }
     ...
     * The goal of this module is to extract abbreviation from current editorвЂ™s line,
     ...
     * abbreviation: itвЂ™s nearest to `pos` location of `prefix` token
     */
    ...
    /**
     * Converts given index range to editorвЂ™s position range
     */
     ...
     *
     * By default, if given location doesnвЂ™t match any known context, this method
    ...
    /**
     * Check if given syntax is a CSS dialect. Note that itвЂ™s not the same as stylesheet
    ...
    class AbbreviationTracker {
        constructor(start, pos, length, params) {
        ...
        serialize() {
            ...
            return {
                range: this.range,
                abbr: (_a = this.abbreviation) === null || _a === void 0 ? void 0 : _a.abbr,
                valid: ((_b = this.abbreviation) === null || _b === void 0 ? void 0 : _b.type) === 'abbreviation',
                forced: this.forced,
                offset: this.offset
            };
        }
    ...
    function startTracking(editor, start, pos, params) {
        const options = (params === null || params === void 0 ? void 0 : params.options) || getOptions(editor, start);
        const tracker = new AbbreviationTracker(start, pos, editor.getValue().length, Object.assign(Object.assign({}, params), { options }));
        tracker.abbreviation = getParsedAbbreviation(editor, [start, pos], options, params === null || params === void 0 ? void 0 : params.offset);
        tracker.mark(editor);
     ...
     */
    function stopTracking(editor, options) {
        const tracker = getTracker(editor);
        if (tracker) {
            tracker.unmark();
            if (tracker.forced && !(options === null || options === void 0 ? void 0 : options.skipRemove)) {
     ...
     */
    function handleChange(editor, pos) {
        const tracker = getTracker(editor);
        if (!tracker) {
            return;
        }
    ...
    function handleSelectionChange(editor, pos) {
    ...
    function getPreviewConfig(config) {
     ...
     */
    function getParsedAbbreviation(editor, range, options, offset) {
        if (range[0] >= range[1]) {
            // Invalid range
            return null;
        }
        ...
        try {
            ...
            if (options.type === 'stylesheet') {
                parsedAbbr = parse(abbr);
            }
            else {
                parsedAbbr = parseAbbreviation(abbr, {
     ...
     */
    function isValidAbbreviation(abbreviation, range, pos) {
     ...
     */
    function restoreTracker(editor, pos) {
        const state = getInternalState(editor);
        const { lastTracker } = state;
        if (lastTracker && lastTracker.range[0] <= pos && lastTracker.range[1] >= pos) {
            // Tracker can be restored at given location. Make sure itвЂ™s contents matches
            // contents of editor at the same location. If it doesnвЂ™t, reset stored tracker
            // since itвЂ™s not valid anymore
    ...
    function extractTracker(editor, pos) {
        ...
        const abbr = extract$1(getContent(editor), pos, getSyntaxType(options === null || options === void 0 ? void 0 : options.syntax), { prefix });
        if (abbr) {
     ...
     */
    function startAbbreviationTracking(editor, pos) {
        var _a, _b;
        ...
        if (start >= 0) {
            // Check if thereвЂ™s paired character
            ...
            if (options) {
                if (options.type === 'stylesheet' && !reStylesheetWordBound.test(prefix)) {
                    // Additional check for stylesheet abbreviation start: itвЂ™s slightly
                ...
                if (((_a = tracker.abbreviation) === null || _a === void 0 ? void 0 : _a.type) === 'abbreviation' && ((_b = options.context) === null || _b === void 0 ? void 0 : _b.name) === "@@section" /* Section */) {
                    ...
                    // e.g. `a` в†’ `a: ;`. If thatвЂ™s the case, stop tracking
     ...
     * itвЂ™s not allowed to expand abbreviations inside `<div ...>` or `</div>`,
     * yet itвЂ™s allowed inside `style` attribute and between tags.
     *
     ...
     */
    function getActivationContext(editor, pos) {
        const syntax = docSyntax(editor);
        if (isCSS(syntax)) {
    ...
    function getCSSActivationContext(editor, pos, syntax, ctx) {
     ...
     * Activate only if itвЂ™s a nested section and itвЂ™s a first character of selector
     */
    function isTypingBeforeSelector(editor, pos, { current }) {
        if (current && current.type === "selector" /* Selector */ && current.range[0] === pos - 1) {
            // Typing abbreviation before selector is tricky one:
            // ensure itвЂ™s on its own line
    ...
    function expandAbbreviationWithTab(editor) {
        ...
        if (getEmmetConfig(editor).mark) {
            const tracker = getTracker(editor);
            if (tracker && tracker.contains(caret) && ((_a = tracker.abbreviation) === null || _a === void 0 ? void 0 : _a.type) === 'abbreviation') {
    ...
    /**
     * Pushes given `range` into `ranges` list on if itвЂ™s not the same as last one
     */
    ...
    function registerEmmetExtension(CM) {
        ...
        CM.defineExtension('getEmmetCompletion', function (pos) {
            ...
            if (tracker && tracker.contains(pos) && ((_a = tracker.abbreviation) === null || _a === void 0 ? void 0 : _a.type) === 'abbreviation') {
                const { abbr, preview } = tracker.abbreviation;
                return {
                    text: abbr,
                    displayText: preview,
